...
export class ThemeMainService extends Disposable implements IThemeMainService {
	...
	private getWindowSplashOverride(): IPartsSplashWorkspaceOverride {
		const item = this.stateService.getItem<IPartsSplashWorkspaceOverride>(THEME_WINDOW_SPLASH_WORKSPACE_OVERRIDE_KEY, { layoutInfo: { auxiliarySideBarWidth: [0, []] } });
		if (!item || !item.layoutInfo || !Array.isArray(item.layoutInfo.auxiliarySideBarWidth) || item.layoutInfo.auxiliarySideBarWidth.length < 2) {
