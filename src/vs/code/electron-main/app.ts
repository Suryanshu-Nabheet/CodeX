...
// Codex channels
import { LLMMessageChannel } from '../../workbench/contrib/codex/electron-main/sendLLMMessageChannel.js';
import { MetricsMainService } from '../../workbench/contrib/codex/electron-main/metricsMainService.js';
import { MCPChannel } from '../../workbench/contrib/codex/electron-main/mcpChannel.js';
import { CodexSCMService } from '../../workbench/contrib/codex/electron-main/codexSCMMainService.js';
import { CodexMainUpdateService } from '../../workbench/contrib/codex/electron-main/codexUpdateMainService.js';
import { IMetricsService } from '../../workbench/contrib/codex/common/metricsService.js';
...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// Codex Metrics
		services.set(IMetricsService, new SyncDescriptor(MetricsMainService));
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// Codex LLM Message Channel
		const codexMetricsService = accessor.get(IMetricsService);
		const llmMessageChannel = new LLMMessageChannel(codexMetricsService);
		mainProcessElectronServer.registerChannel('codex-channel-llmMessage', llmMessageChannel);

		// Codex Metrics Channel
		const metricsChannel = ProxyChannel.fromService(codexMetricsService, disposables);
		mainProcessElectronServer.registerChannel('codex-channel-metrics', metricsChannel);

		// Codex MCP Channel
		const codexMCPChannel = new MCPChannel();
		mainProcessElectronServer.registerChannel('codex-channel-mcp', codexMCPChannel);

		// Codex SCM Channel
		const codexSCMService = new CodexSCMService();
		const codexSCMChannel = ProxyChannel.fromService(codexSCMService, disposables);
		mainProcessElectronServer.registerChannel('codex-channel-scm', codexSCMChannel);

		// Codex Update Channel
		const codexUpdateService = accessor.get(IInstantiationService).createInstance(CodexMainUpdateService);
		const codexUpdateChannel = ProxyChannel.fromService(codexUpdateService, disposables);
		mainProcessElectronServer.registerChannel('codex-channel-update', codexUpdateChannel);
	}
