	...
	override async run(accessor: ServicesAccessor) {
		...
		const options = {
			...
			onToggle: async (toggle, picker) => {
				picker.hide();
				const isEnabled = configurationService.getValue(ThemeSettings.DETECT_COLOR_SCHEME);
				await configurationService.updateValue(ThemeSettings.DETECT_COLOR_SCHEME, !isEnabled);
			}
		...
		const effectiveColorScheme = (preferredColorScheme ?? (isHighContrast(currentTheme.type) ? ColorScheme.HIGH_CONTRAST_DARK : currentTheme.type));
	...
	override async run(accessor: ServicesAccessor) {
		...
		if (configurationService.getValue(ThemeSettings.DETECT_COLOR_SCHEME)) {
			...
			notificationService.prompt(Severity.Info, message, [
				{
					label: localize('toggleAnyway', "Turn Off & Toggle"),
					run: async () => {
		...
		let targetType: ColorScheme;
		let preferredSettingId: string;

		switch (currentTheme.type) {
			case ColorScheme.LIGHT:
				targetType = ColorScheme.DARK;
				preferredSettingId = ThemeSettings.PREFERRED_DARK_THEME;
				break;
			case ColorScheme.DARK:
				targetType = ColorScheme.LIGHT;
				preferredSettingId = ThemeSettings.PREFERRED_LIGHT_THEME;
				break;
			case ColorScheme.HIGH_CONTRAST_LIGHT:
				targetType = ColorScheme.HIGH_CONTRAST_DARK;
				preferredSettingId = ThemeSettings.PREFERRED_HC_DARK_THEME;
				break;
			case ColorScheme.HIGH_CONTRAST_DARK:
				targetType = ColorScheme.HIGH_CONTRAST_LIGHT;
				preferredSettingId = ThemeSettings.PREFERRED_HC_LIGHT_THEME;
				break;
			default:
				targetType = ColorScheme.LIGHT;
				preferredSettingId = ThemeSettings.PREFERRED_LIGHT_THEME;
		...
		const themeSettingId: string = configurationService.getValue(preferredSettingId);
		let theme = themes.find(t => t.settingsId === themeSettingId);

		if (!theme || theme.id === currentTheme.id) {
			const defaultId = targetType === ColorScheme.LIGHT ? ThemeSettingDefaults.COLOR_THEME_LIGHT : ThemeSettingDefaults.COLOR_THEME_DARK;
			theme = themes.find(t => t.settingsId === defaultId);
		}

		if (!theme || theme.id === currentTheme.id) {
			theme = themes.find(t => t.type === targetType && t.id !== currentTheme.id);
		}
