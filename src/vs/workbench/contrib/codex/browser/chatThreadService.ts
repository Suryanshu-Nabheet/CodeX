...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/codexSettingsTypes.js';
...
import { CodexFileSnapshot } from '../common/editCodeServiceTypes.js';
...
export const IChatThreadService = createDecorator<IChatThreadService>('codexChatThreadService');
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	// should probably re-use code from codex/src/vs/base/common/marshalling.ts instead. but this is simple enough
	...
	private _setState(state: Partial<ThreadsState>, doNotRefreshMountInfo?: boolean) {
		...
		if (streamState?.isRunning === undefined && !streamState?.error) {
			...
			// if awaiting user but stream state doesn't indicate it (happens if restart Codex)
			if (lastMessage && lastMessage.role === 'tool' && lastMessage.type === 'tool_request')
				this._setStreamState(threadId, { isRunning: 'awaiting_user', })

			// if running now but stream state doesn't indicate it (happens if restart Codex), cancel that last tool
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		const codexFileSnapshot = checkpointMessage.codexFileSnapshotOfURI ? checkpointMessage.codexFileSnapshotOfURI[fsPath] ?? null : null
		if (!opts.includeUserModifiedChanges) { return { codexFileSnapshot, } }

		const userModifiedCodexFileSnapshot = fsPath in checkpointMessage.userModifications.codexFileSnapshotOfURI ? checkpointMessage.userModifications.codexFileSnapshotOfURI[fsPath] ?? null : null
		return { codexFileSnapshot: userModifiedCodexFileSnapshot ?? codexFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const codexFileSnapshotOfURI: { [fsPath: string]: CodexFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			const { model } = this._codexModelService.getModelFromFsPath(fsPath)
			if (!model) continue
			...
			if (!res) continue
			const { codexFileSnapshot: oldCodexFileSnapshot } = res
			...
			const codexFileSnapshot = this._editCodeService.getCodexFileSnapshot(URI.file(fsPath))
			if (oldCodexFileSnapshot === codexFileSnapshot) continue
			codexFileSnapshotOfURI[fsPath] = codexFileSnapshot
		}
		...
		return { codexFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { codexFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			codexFileSnapshotOfURI: codexFileSnapshotOfURI ?? {},
			userModifications: { codexFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getCodexFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			codexFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { codexFileSnapshotOfURI: {} },
		})
	}
	...
	private _getCheckpointsBetween({ threadId, loIdx, hiIdx }: { threadId: string, loIdx: number, hiIdx: number }) {
		...
		for (let i = loIdx; i <= hiIdx; i += 1) {
			...
			for (const fsPath in message.codexFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { codexFileSnapshotOfURI: codexFileSnapshotOfURI ?? {}, },
		})
	}
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					if (!res) continue
					const { codexFileSnapshot } = res
					if (!codexFileSnapshot) continue
					this._editCodeService.restoreCodexFileSnapshot(URI.file(fsPath), codexFileSnapshot)
					break
				}
			}
		}
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._codexModelService.getModelSafe(uri)
