...
import { mountCodexSelectionHelper } from './react/out/codex-editor-widgets-tsx/index.js';
...
export type CodexSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	public static readonly ID = 'editor.contrib.codexSelectionHelper';
	// react
	...
	constructor(
		...
		@ICodexSettingsService private readonly _codexSettingsService: ICodexSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountCodexSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._codexSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies CodexSelectionHelperProps)
