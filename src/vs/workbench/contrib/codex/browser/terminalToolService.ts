...
export const persistentTerminalNameOfId = (id: string) => {
	if (id === '1') return 'Codex Agent'
	return `Codex Agent (${id})`
}
export const idOfPersistentTerminalName = (name: string) => {
	if (name === 'Codex Agent') return '1'

	const match = name.match(/Codex Agent \((\d+)\)/)
...
export class TerminalToolService extends Disposable implements ITerminalToolService {
	...
	runCommand: ITerminalToolService['runCommand'] = async (command, params) => {
		...
		const waitForResult = async () => {
			...
			// if (!cmdCap) throw new Error(`There was an error using the terminal: CommandDetection capability did not mount yet. Please try again in a few seconds or report this to the Suryanshu Nabheet.`)
