...
import { codexPrefixAndSuffix, ctrlKStream_userMessage, ctrlKStream_systemMessage, defaultQuickEditFimTags, rewriteCode_systemMessage, rewriteCode_userMessage, searchReplaceGivenDescription_systemMessage, searchReplaceGivenDescription_userMessage, tripleTick, } from '../common/prompt/prompts.js';
import { ICodexCommandBarService } from './ICodexCommandBarService.js';
import { IKeybindingService } from '../../../../platform/keybinding/common/keybinding.js';
import { CODEX_ACCEPT_DIFF_ACTION_ID, CODEX_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { DiffArea, Diff, CtrlKZone, CodexFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@ICodexModelService private readonly _codexModelService: ICodexModelService,
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._codexModelService.initializeModel(model.uri)
	...
	// 		message: `Codex Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'codex.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open Codex's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(CODEX_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/Suryanshu-Nabheet/CodeX/issues/new) it.` : undefined
	// 	})
	// }
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		const { model } = this._codexModelService.getModel(uri)
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'codex-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'codex-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'codex-highlightBG')
	...
	private _addDiffStylesToURI = (uri: URI, diff: Diff) => {
		...
		if (type !== 'deletion') {
			const fn = this._addLineDecoration(model, diff.startLine, diff.endLine, 'codex-greenBG', {
	...
	private _getCurrentCodexFileSnapshot = (uri: URI): CodexFileSnapshot => {
		const { model } = this._codexModelService.getModel(uri)
	...
	private _restoreCodexFileSnapshot = async (uri: URI, snapshot: CodexFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: CodexFileSnapshot = this._getCurrentCodexFileSnapshot(uri)
		let afterSnapshot: CodexFileSnapshot | null = null
		...
		const elt: IUndoRedoElement = {
			...
			undo: async () => { opts?.onWillUndo?.(); await this._restoreCodexFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreCodexFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentCodexFileSnapshot(uri)
			await this._codexModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getCodexFileSnapshot(uri: URI) {
		return this._getCurrentCodexFileSnapshot(uri)
	}


	public restoreCodexFileSnapshot(uri: URI, snapshot: CodexFileSnapshot): void {
		this._restoreCodexFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Codex: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._codexModelService.initializeModel(uri)
		await this._codexModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Codex: diff.type not recognized on: ${from}`)
		}
		...
		if (from === 'ClickApply') {
			...
			const { prefix, suffix } = codexPrefixAndSuffix({ fullFileStr: originalFileCode, startLine, endLine })
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Codex: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
	...
	public async acceptDiff({ diffid }: { diffid: number }) {
		...
		if (diff.type === 'deletion') {
			...
			throw new Error(`Codex error: ${diff}.type not recognized`)
		}
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@ICodexCommandBarService private readonly _codexCommandBarService: ICodexCommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(CODEX_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(CODEX_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._codexCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._codexCommandBarService.onDidChangeState(e => {
