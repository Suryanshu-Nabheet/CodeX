...
function saveStylesFile() {
	setTimeout(() => {
		try {
			const pathToCssFile = findDesiredPathFromLocalPath('./src/vs/workbench/contrib/codex/browser/react/src2/styles.css', __dirname);
...
if (isWatch) {
	...
	if (!fs.existsSync('src2')) {
		try {
			...
			execSync(
				'npx scope-tailwind ./src -o src2/ -s codex-scope -c styles.css -p "codex-"',
				{ stdio: 'inherit' }
			);
	...
	execSync('npx scope-tailwind ./src -o src2/ -s codex-scope -c styles.css -p "codex-"', { stdio: 'inherit' });
