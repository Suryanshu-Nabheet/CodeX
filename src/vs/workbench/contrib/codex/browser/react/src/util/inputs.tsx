...
export const CodexInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
...
export const CodexSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps }: {
	value: string;
...
export const CodexInputBox = ({ onChangeText, onCreateInstance, inputBoxRef, placeholder, isPasswordField, multiline }: {
	onChangeText: (value: string) => void;
	...
	return <WidgetComponent
		className='
			bg-codex-bg-1
			@@codex-force-child-placeholder-codex-fg-1
		'
		ctor={InputBox}
				...
				<div className="relative w-full">
					...
					<div
						className={`relative ${size === 'xxs' ? 'h-0.5' :
							size === 'xs' ? 'h-1' :
								size === 'sm' ? 'h-1.5' :
									size === 'sm+' ? 'h-2' : 'h-2.5'
							} bg-codex-bg-2 rounded-full cursor-pointer`}
						onClick={handleTrackClick}
					>
					...
					<div
						className={`absolute top-1/2 transform -translate-x-1/2 -translate-y-1/2
							...
							bg-codex-fg-1 rounded-full shadow-md ${disabled ? 'cursor-not-allowed' : 'cursor-grab active:cursor-grabbing'}
							border border-codex-fg-1`}
...
export const CodexCheckBox = ({ label, value, onClick, className }: { label: string, value: boolean, onClick: (checked: boolean) => void, className?: string }) => {
...
export const CodexCustomDropdownBox = <T extends NonNullable<any>>({
	options,
	selectedOption,
	onChangeOption,
...
export const _CodexSelectBox = <T,>({ onChangeSelection, onCreateInstance, selectBoxRef, options, className }: {
	...
	return <WidgetComponent
		className={`
			...
			@@[&_select]:!codex-text-codex-fg-3
			@@[&_select]:!codex-text-xs
			!text-codex-fg-3
			${className ?? ''}
		`}
		ctor={SelectBox}
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
	...
	return <div ref={divRef} className='relative z-0 px-2 py-1 bg-codex-bg-3'>
		<WidgetComponent
...
export const CodexButtonBgDarken = ({ children, disabled, onClick, className }: { children: React.ReactNode; disabled?: boolean; onClick: () => void; className?: string }) => {
...
// export const CodexScrollableElt = ({ options, children }: { options: ScrollableElementCreationOptions, children: React.ReactNode }) => {
...
// export const CodexSelectBox = <T,>({ onChangeSelection, initVal, selectBoxRef, options }: {
// 	initVal: T;
// 	selectBoxRef: React.MutableRefObject<SelectBox | null>;
// 	options: readonly { text: string, value: T }[];
// 	onChangeSelection: (value: T) => void;
...
// export const CodexCheckBox = ({ onChangeChecked, initVal, label, checkboxRef, }: {
...
const SingleDiffEditor = ({ block, lang }: { block: ExtractedSearchReplaceBlock, lang: string | undefined }) => {
	...
	return (
		<div className="w-full bg-codex-bg-3 @@bg-editor-style-override" ref={divRef} />
	);
};
 ...
 */
export const CodexDiffEditor = ({ uri, searchReplaceBlocks, language }: { uri?: any, searchReplaceBlocks: string, language?: string }) => {
	const accessor = useAccessor();
	const languageService = accessor.get('ILanguageService');
	...
	if (blocks.length === 0) {
		return <div className="w-full p-4 text-codex-fg-4 text-sm">No changes found</div>;
	}
		...
		<div className="w-full flex flex-col gap-2">
			{blocks.map((block, index) => (
				<div key={index} className="w-full">
					{blocks.length > 1 && (
						<div className="text-codex-fg-4 text-xs mb-1 px-1">
