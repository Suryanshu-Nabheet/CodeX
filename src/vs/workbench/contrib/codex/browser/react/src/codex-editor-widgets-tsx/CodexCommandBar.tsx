...
import { CodexCommandBarProps } from '../../../codexCommandBarService.js';
import { Check, EllipsisVertical, Menu, MoveDown, MoveLeft, MoveRight, MoveUp, X } from 'lucide-react';
import {
	CODEX_GOTO_NEXT_DIFF_ACTION_ID,
	CODEX_GOTO_PREV_DIFF_ACTION_ID,
	CODEX_GOTO_NEXT_URI_ACTION_ID,
	CODEX_GOTO_PREV_URI_ACTION_ID,
	CODEX_ACCEPT_FILE_ACTION_ID,
	CODEX_REJECT_FILE_ACTION_ID,
	CODEX_ACCEPT_ALL_DIFFS_ACTION_ID,
	CODEX_REJECT_ALL_DIFFS_ACTION_ID
} from '../../../actionIDs.js';

export const CodexCommandBarMain = ({ uri, editor }: CodexCommandBarProps) => {
	const isDark = useIsDark()

	return <div
		className={`@@codex-scope ${isDark ? 'dark' : ''}`}
	>
		<CodexCommandBar uri={uri} editor={editor} />
	</div>
}
...
export const CodexCommandBar = ({ uri, editor }: CodexCommandBarProps) => {
	...
	const codexModelService = accessor.get('ICodexModelService')
	...
	const _upKeybinding = keybindingService.lookupKeybinding(CODEX_GOTO_PREV_DIFF_ACTION_ID);
	const _downKeybinding = keybindingService.lookupKeybinding(CODEX_GOTO_NEXT_DIFF_ACTION_ID);
	const _leftKeybinding = keybindingService.lookupKeybinding(CODEX_GOTO_PREV_URI_ACTION_ID);
	const _rightKeybinding = keybindingService.lookupKeybinding(CODEX_GOTO_NEXT_URI_ACTION_ID);
	const _acceptFileKeybinding = keybindingService.lookupKeybinding(CODEX_ACCEPT_FILE_ACTION_ID);
	const _rejectFileKeybinding = keybindingService.lookupKeybinding(CODEX_REJECT_FILE_ACTION_ID);
	const _acceptAllKeybinding = keybindingService.lookupKeybinding(CODEX_ACCEPT_ALL_DIFFS_ACTION_ID);
	const _rejectAllKeybinding = keybindingService.lookupKeybinding(CODEX_REJECT_ALL_DIFFS_ACTION_ID);
	...
	if (currFileIdx === null) {
		return (
			<div className="pointer-events-auto">
				<div className="flex bg-codex-bg-2 shadow-md border border-codex-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:border-r [&>*]:border-codex-border-2 [&>*:last-child]:border-r-0">
		...
		<div className="pointer-events-auto">
			...
			{showAcceptRejectAllButtons && showAcceptRejectAll && (
				<div className="flex justify-end mb-1">
					<div className="inline-flex bg-codex-bg-2 rounded shadow-md border border-codex-border-2 overflow-hidden">
						<div className="flex items-center [&>*]:border-r [&>*]:border-codex-border-2 [&>*:last-child]:border-r-0">
			...
			<div className="flex items-center bg-codex-bg-2 rounded shadow-md border border-codex-border-2 [&>*:first-child]:pl-3 [&>*:last-child]:pr-3 [&>*]:px-3 [&>*]:border-r [&>*]:border-codex-border-2 [&>*:last-child]:border-r-0">
