...
export const PastThreadsList = ({ className = '' }: { className?: string }) => {
	...
	return (
		<div className={`flex flex-col mb-2 gap-2 w-full text-nowrap text-codex-fg-3 select-none relative ${className}`}>
			...
			{hasMoreThreads && !showAll && (
				<div
					className="text-codex-fg-3 opacity-80 hover:opacity-100 hover:brightness-115 cursor-pointer p-1 text-xs"
...
const PastThreadElement = ({ pastThread, idx, hoveredIdx, setHoveredIdx, isRunning }: {
	...
	// 	}}
	// 	toolTipName={`Copy As Codex Chat`}
	// />
		...
		<div className="flex items-center justify-between gap-1">
			<span className="flex items-center gap-2 min-w-0 overflow-hidden">
				...
				{isRunning === 'LLM' || isRunning === 'tool' || isRunning === 'idle' ? <LoaderCircle className="animate-spin bg-codex-stroke-1 flex-shrink-0 flex-grow-0" size={14} />
					:
					isRunning === 'awaiting_user' ? <MessageCircleQuestion className="bg-codex-stroke-1 flex-shrink-0 flex-grow-0" size={14} />
						:
						null}
				{/* name */}
