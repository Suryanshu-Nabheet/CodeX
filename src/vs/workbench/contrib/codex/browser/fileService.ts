...
class FilePromptActionService extends Action2 {
	private static readonly CODEX_COPY_FILE_PROMPT_ID = 'codex.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.CODEX_COPY_FILE_PROMPT_ID,
			title: localize2('codexCopyPrompt', 'Codex: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await codexModelService.getModelSafe(uri)).model?.getLanguageId() || '',
