...
const extensionBlacklist = [
	...
	// ignore other AI copilots that could conflict with Codex keybindings
	'sourcegraph.cody-ai',
	'continue.continue',
	'codeium.codeium',
	...
	if (os === 'mac') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Codex', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Codex', 'User', 'keybindings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.vscode', 'extensions'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.codex-editor', 'extensions'),
				isExtensions: true,
			}]
	...
	if (os === 'linux') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Codex', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Codex', 'User', 'keybindings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.vscode', 'extensions'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.codex-editor', 'extensions'),
				isExtensions: true,
	...
	if (os === 'windows') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Codex', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Codex', 'User', 'keybindings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), userprofile, '.vscode', 'extensions'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), userprofile, '.codex-editor', 'extensions'),
				isExtensions: true,
			}]
		} else if (fromEditor === 'Cursor') {
			return [{
