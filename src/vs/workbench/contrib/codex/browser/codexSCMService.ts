...
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/codexSettingsTypes.js'
...
export const IGenerateCommitMessageService = createDecorator<IGenerateCommitMessageService>('codexGenerateCommitMessageService');

const loadingContextKey = 'codexSCMGenerateCommitMessageLoading'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private codexSCM: ICodexSCMService
	...
	constructor(
		...
		this.codexSCM = ProxyChannel.toService<ICodexSCMService>(mainProcessService.getChannel('codex-channel-scm'))
	}
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const [stat, sampledDiffs, branch, log] = await Promise.all([
					this.codexSCM.gitStat(path),
					this.codexSCM.gitSampledDiffs(path),
					this.codexSCM.gitBranch(path),
					this.codexSCM.gitLog(path)
				])

				if (!this.isCurrentRequest(requestId)) { throw new CancellationError() }

				const modelSelection = this.codexSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.codexSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.codexSettingsService.state.overridesOfModel
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'CodexSCM - Commit Message' },
			})
		})
	}
	...
	private onError(error: any) {
		if (!isCancellationError(error)) {
			console.error(error)
			this.notificationService.error(localize2('codexFailedToGenerateCommitMessage', 'Failed to generate commit message.').value)
		}
	}
}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'codex.generateCommitMessageAction',
			title: localize2('codexCommitMessagePrompt', 'Codex: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('codexCommitMessagePromptTooltip', 'Codex: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'codex.loadingGenerateCommitMessageAction',
			title: localize2('codexCommitMessagePromptCancel', 'Codex: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('codexCommitMessagePromptCancelTooltip', 'Codex: Cancel Commit Message Generation'),
