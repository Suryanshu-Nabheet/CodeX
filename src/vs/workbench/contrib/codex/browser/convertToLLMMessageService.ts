...
import { ChatMode, FeatureName, ModelSelection, ProviderName } from '../common/codexSettingsTypes.js';
...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .codexrules file):\n${aiInstructions}`)
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	// Read .codexrules files from workspace folders
	private _getCodexRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let codexRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.codexrules')
				const { model } = this.codexModelService.getModel(uri)
				if (!model) continue
				codexRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return codexRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	// Get combined AI instructions from settings and .codexrules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.codexSettingsService.state.globalSettings.aiInstructions;
		const codexRulesFileContent = this._getCodexRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (codexRulesFileContent) ans.push(codexRulesFileContent)
		return ans.join('\n\n')
	}
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.codexSettingsService.state
		...
		const modelSelectionOptions = this.codexSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.codexSettingsService.state.globalSettings;
		...
		const modelSelectionOptions = this.codexSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareFIMMessage: IConvertToLLMMessageService['prepareFIMMessage'] = ({ messages }) => {
		// Production FIM Protocol:
		// For FIM mode, we prioritize a raw, instruction-less prefix to prevent the model
