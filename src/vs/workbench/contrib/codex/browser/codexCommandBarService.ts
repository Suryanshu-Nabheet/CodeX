...
import { ICodexCommandBarService, CommandBarStateType, CodexCommandBarProps } from './ICodexCommandBarService.js';
export { ICodexCommandBarService, CommandBarStateType, CodexCommandBarProps };
import { mountCodexCommandBar } from './react/out/codex-editor-widgets-tsx/index.js'
...
import { CODEX_ACCEPT_DIFF_ACTION_ID, CODEX_REJECT_DIFF_ACTION_ID, CODEX_GOTO_NEXT_DIFF_ACTION_ID, CODEX_GOTO_PREV_DIFF_ACTION_ID, CODEX_GOTO_NEXT_URI_ACTION_ID, CODEX_GOTO_PREV_URI_ACTION_ID, CODEX_ACCEPT_FILE_ACTION_ID, CODEX_REJECT_FILE_ACTION_ID, CODEX_ACCEPT_ALL_DIFFS_ACTION_ID, CODEX_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export class CodexCommandBarService extends Disposable implements ICodexCommandBarService {
	_serviceBrand: undefined;

	static readonly ID: 'codex.CodexCommandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._codexModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(ICodexCommandBarService, CodexCommandBarService, InstantiationType.Delayed); // delayed is needed here :(
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountCodexCommandBar(root, accessor, { uri, editor } satisfies CodexCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies CodexCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODEX_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('codexAcceptDiffAction', 'Codex: Accept Diff'),
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyMod.Shift | KeyCode.Enter,
				mac: { primary: KeyMod.WinCtrl | KeyMod.Alt | KeyCode.Enter },
				weight: KeybindingWeight.CodexExtension,
			}
		});
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODEX_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('codexRejectDiffAction', 'Codex: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODEX_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('codexGoToNextDiffAction', 'Codex: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODEX_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('codexGoToPrevDiffAction', 'Codex: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODEX_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('codexGoToNextUriAction', 'Codex: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODEX_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('codexGoToPrevUriAction', 'Codex: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODEX_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('codexAcceptFileAction', 'Codex: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODEX_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('codexRejectFileAction', 'Codex: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODEX_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('codexAcceptAllDiffsAction', 'Codex: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODEX_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('codexRejectAllDiffsAction', 'Codex: Reject All Diffs in All Files'),
			keybinding: {
