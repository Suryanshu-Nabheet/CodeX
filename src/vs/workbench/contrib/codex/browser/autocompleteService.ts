...
const DEBOUNCE_TIME = 25; // Optimized for high-frequency typing
const TIMEOUT_TIME = 60000;
const MAX_CACHE_SIZE = 150; // High-capacity LRU for document-level continuity
const MAX_PENDING_REQUESTS = 12; // Maximum concurrent predictive payloads
...
const getCompletionOptions = (prefixAndSuffix: PrefixAndSuffixInfo, relevantContext: string, justAcceptedAutocompletion: boolean): CompletionOptions => {
	...
	suffixLines = suffix.split(_ln).slice(0, 100)
	prefixLines = prefix.split(_ln).slice(-100)
	...
	if (justAcceptedAutocompletion && isLineSuffixEmpty) {
		const prefixWithNewline = llmPrefixWithContext + _ln
		completionOptions = {
...
export class AutocompleteService extends Disposable implements IAutocompleteService {
	...
	async _provideInlineCompletionItems(
		...
		this._contextGatheringService.updateCache(model, position); // Do not await, use whatever is in the cache for maximum speed
		const relevantSnippetsList = this._contextGatheringService.getCachedSnippets();
		const recentHistory = this._recentCompletionsBuffer.length > 0 ? `/* Recent work history (continuous context):\n${this._recentCompletionsBuffer.join('\n---\n')}\n*/\n` : '';
		...
		newAutocompletion.llmPromise = new Promise((resolve, reject) => {
			...
			const requestId = this._llmMessageService.sendLLMMessage({
				...
				onFinalMessage: ({ fullText }) => {
					...
					// handle special case for predicting starting on the next line
			...
			freeInlineCompletions: (completions) => {
				...
				this._autocompletionsOfDocument[docUriStr].items.forEach((autocompletion: Autocompletion) => {
					...
					if (matchup) {
						...
						// Add to continuous work history buffer
						this._recentCompletionsBuffer.push(autocompletion.insertText);
						if (this._recentCompletionsBuffer.length > 5) this._recentCompletionsBuffer.shift(); // Keep last 5 for architectural continuity
