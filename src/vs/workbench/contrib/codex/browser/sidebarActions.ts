...
import { CODEX_VIEW_CONTAINER_ID, CODEX_VIEW_ID } from './sidebarPane.js';
...
const CODEX_OPEN_SIDEBAR_ACTION_ID = 'codex.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: CODEX_OPEN_SIDEBAR_ACTION_ID, title: localize2('codexOpenSidebar', 'Codex: Open Sidebar'), f1: true });
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const viewsService = accessor.get(IViewsService)
		const chatThreadsService = accessor.get(IChatThreadService)
		viewsService.openViewContainer(CODEX_VIEW_CONTAINER_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODEX_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('codexCmdL', 'Codex: Add Selection to Chat'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(CODEX_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(CODEX_OPEN_SIDEBAR_ACTION_ID)
		}
...
const CODEX_CMD_SHIFT_L_ACTION_ID = 'codex.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: CODEX_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', CODEX_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(CODEX_CMD_SHIFT_L_ACTION_ID)
...
// Removed codex.settingsAction from ViewTitle to reduce clutter as requested by user
