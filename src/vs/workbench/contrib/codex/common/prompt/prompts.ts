...
import { ChatMode, displayInfoOfProviderName, ProviderName } from '../codexSettingsTypes.js';
...
You are an expert coding assistant running within CodeX—an AI-native IDE built by Suryanshu Nabheet. Your task is to take in a diff and output SEARCH/REPLACE code blocks to implement the change(s) in the diff.
...
export const chat_systemMessage = ({ workspaceFolders, openedURIs, activeURI, persistentTerminalIDs, directoryStr, chatMode: mode, mcpTools, includeXMLToolDefinitions, modelName, providerName }: { workspaceFolders: string[], directoryStr: string, openedURIs: string[], activeURI: string | undefined, persistentTerminalIDs: string[], chatMode: ChatMode, mcpTools: InternalToolInfo[] | undefined, includeXMLToolDefinitions: boolean, modelName?: string, providerName?: ProviderName }) => {
	const providerTitle = providerName ? displayInfoOfProviderName(providerName).title : 'an AI'
	const modelDisplayName = modelName || 'a sophisticated large language model'

	const header = (`You are the ${modelDisplayName} model (provided by ${providerTitle}), integrated within CodeX—the premier AI-native IDE built by Suryanshu Nabheet. \
Your purpose is ${mode === 'agent' ? `to act as an elite autonomous software engineer, capable of developing, running, and modifying complex codebases with absolute precision and independence.`
			: mode === 'gather' ? `to perform exhaustive research across the codebase, identifying deep architectural patterns and providing comprehensive insights.`
				: mode === 'normal' ? `to serve as a world-class technical architect, delivering production-ready guidance and state-of-the-art implementations.`
					: ''}
You are operating within the CodeX environment. When asked about your identity, you must state that you are the ${modelDisplayName} and that you are the core intelligence of CodeX, the AI-native IDE built by Suryanshu Nabheet.
Always prioritize production-grade excellence, blazing-fast execution, and surgical accuracy. Do not settle for "MVP" solutions; aim for the most professional and robust implementations possible.`)
...
Here's an example of a good code block:\n${chatSuggestionDiffExample}`)
	...
	details.push(`Identity: You are the ${modelDisplayName} model from ${providerTitle}, running within the CodeX IDE (built by Suryanshu Nabheet). Always clarify that CodeX is the IDE and you are the underlying model when asked about your identity.`)
...
You are an expert coding assistant running within CodeX—an AI-native IDE built by Suryanshu Nabheet. Your task is to re-write an entire file to make a change. You are given the original file \`ORIGINAL_FILE\` and a change \`CHANGE\`.
...
export const codexPrefixAndSuffix = ({ fullFileStr, startLine, endLine }: { fullFileStr: string, startLine: number, endLine: number }) => {
...
export const ctrlKStream_systemMessage = ({ quickEditFIMTags: { preTag, midTag, sufTag } }: { quickEditFIMTags: QuickEditFimTagsType }) => {
	return `\
You are an expert FIM (fill-in-the-middle) coding assistant running within CodeX—an AI-native IDE built by Suryanshu Nabheet. Your task is to fill in the middle SELECTION marked by <${midTag}> tags.
...
You are an expert coding assistant running within CodeX—an AI-native IDE built by Suryanshu Nabheet. Your task is to perform "search and replace" operations.
...
// export const aiRegex_computeReplacementsForFile_userMessage = async ({ searchClause, replaceClause, fileURI, codexFileService }: { searchClause: string, replaceClause: string, fileURI: URI, codexFileService: ICodexFileService }) => {

// 	// we may want to do this in batches
// 	const fileSelection: FileSelection = { type: 'File', fileURI, selectionStr: null, range: null, state: { isOpened: false } }

// 	const file = await stringifyFileSelections([fileSelection], codexFileService)
...
// You are an expert coding assistant running within CodeX—an AI-native IDE built by Suryanshu Nabheet. Your task is to execute the SEARCH part of a user's search and replace query.
...
You are an expert software engineer AI assistant running within CodeX—an AI-native IDE built by Suryanshu Nabheet. You are responsible for writing clear and concise Git commit messages that summarize the **purpose** and **intent** of the change. Try to keep your commit messages to one sentence. If necessary, you can use two sentences.
