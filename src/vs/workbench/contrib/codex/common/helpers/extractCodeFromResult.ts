...
const codexSubstr = (str: string, start: number, end: number) => end < start ? '' : str.substring(start, end)
...
export const extractSearchReplaceBlocks = (str: string) => {
	...
	while (true) {
		...
		if (dividerStart === -1) { // if didnt find DIVIDER_, either writing originalStr or DIVIDER_ right now
			...
			blocks.push({
				orig: codexSubstr(str, origStart, str.length - writingDIVIDERlen),
				final: '',
				state: 'writingOriginal'
			})
			return blocks
		}
		const origStrDone = codexSubstr(str, origStart, dividerStart)
		...
		if (finalStart === -1) { // if didnt find FINAL_, either writing finalStr or FINAL or FINAL_ right now
			...
			blocks.push({
				orig: origStrDone,
				final: codexSubstr(str, dividerStart, str.length - usingWritingFINALlen),
				state: 'writingFinal'
			})
			return blocks
		}
		const usingFINAL = matchedFullFINAL_ ? '\n' + FINAL : FINAL
		const finalStrDone = codexSubstr(str, dividerStart, finalStart)
