...
import { CODEX_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ProviderName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, providerNames, ModelSelection, modelSelectionsEqual, featureNames, CodexStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './codexSettingsTypes.js';
...
// type RealCodexSettings = Exclude<keyof CodexSettingsState, '_modelOptions'>
// type EventProp<T extends RealCodexSettings = RealCodexSettings> = T extends 'globalSettings' ? [T, keyof CodexSettingsState[T]] : T | 'all'


export interface ICodexSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: CodexSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: CodexSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: CodexStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, CodexStatefulModelInfo> = {}
...
export const modelFilterOfFeatureName: {
	...
	'Autocomplete': { filter: o => true, emptyMessage: null },
...
const _stateWithMergedDefaultModels = (state: CodexSettingsState): CodexSettingsState => {
...
const _validatedModelState = (state: Omit<CodexSettingsState, '_modelOptions'>): CodexSettingsState => {
...
export const ICodexSettingsService = createDecorator<ICodexSettingsService>('CodexSettingsService');
class CodexSettingsService extends Disposable implements ICodexSettingsService {
	...
	dangerousSetState = async (newState: CodexSettingsState) => {
	...
	private async _readState(): Promise<CodexSettingsState> {
		const encryptedState = this._storageService.get(CODEX_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(CODEX_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: CodexSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: CodexStatefulModelInfo[] = [
...
registerSingleton(ICodexSettingsService, CodexSettingsService, InstantiationType.Eager);
