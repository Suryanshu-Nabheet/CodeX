...
import { MCPUserStateOfName } from './codexSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	constructor(
		...
		super();
		this.channel = this.mainProcessService.getChannel('codex-channel-mcp')
	...
	private async _initialize() {
		try {
			await this.codexSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.codexSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.codexSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.codexSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	// toggle MCP server and update isOn in codex settings
