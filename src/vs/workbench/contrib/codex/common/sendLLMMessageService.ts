...
export class LLMMessageService extends Disposable implements ILLMMessageService {
	...
	constructor(
		...
		// const service = ProxyChannel.toService<LLMMessageChannel>(mainProcessService.getChannel('codex-channel-sendLLMMessage')); // lets you call it like a service
		// see llmMessageChannel.ts
		this.channel = this.mainProcessService.getChannel('codex-channel-llmMessage')
	...
	sendLLMMessage(params: ServiceSendLLMMessageParams) {
		...
		if (modelSelection === null) {
			const message = `Please add a provider in Codex's Settings.`
		...
		const { settingsOfProvider, } = this.codexSettingsService.state
