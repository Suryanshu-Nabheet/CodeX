...
import { ICodexSettingsService } from './codexSettingsService.js';
import { ILLMMessageService } from './sendLLMMessageService.js';
import { Emitter, Event } from '../../../../base/common/event.js';
import { Disposable, IDisposable } from '../../../../base/common/lifecycle.js';
import { RefreshableProviderName, refreshableProviderNames, SettingsOfProvider } from './codexSettingsTypes.js';
...
export class RefreshModelService extends Disposable implements IRefreshModelService {
	...
	constructor(
		...
		const initializeAutoPollingAndOnChange = () => {
			...
			if (!codexSettingsService.state.globalSettings.autoRefreshModels) return

			for (const providerName of refreshableProviderNames) {

				// const { '_didFillInProviderSettings': enabled } = this.codexSettingsService.state.settingsOfProvider[providerName]
				this.startRefreshingModels(providerName, autoOptions)

				// every time providerName.enabled changes, refresh models too, like a useEffect
				let relevantVals = () => refreshBasedOn[providerName].map(settingName => codexSettingsService.state.settingsOfProvider[providerName][settingName])
				let prevVals = relevantVals() // each iteration of a for loop has its own context and vars, so this is ok
				disposables.add(
					codexSettingsService.onDidChangeState(() => { // we might want to debounce this
		...
		codexSettingsService.waitForInitState.then(() => {
			initializeAutoPollingAndOnChange()
			this._register(
				codexSettingsService.onDidChangeState((type) => { if (typeof type === 'object' && type[1] === 'autoRefreshModels') initializeAutoPollingAndOnChange() })
			)
		})
	...
	startRefreshingModels: IRefreshModelService['startRefreshingModels'] = (providerName, options) => {
		...
		const autoPoll = () => {
			if (this.codexSettingsService.state.globalSettings.autoRefreshModels) {
				// resume auto-polling
		...
		listFn({
			...
			onSuccess: ({ models }) => {
				...
				this.codexSettingsService.setAutodetectedModels(
					providerName,
					models.map(model => {
				...
				if (options.enableProviderOnSuccess) this.codexSettingsService.setSettingOfProvider(providerName, '_didFillInProviderSettings', true)
