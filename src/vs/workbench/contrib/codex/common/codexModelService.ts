...
export interface ICodexModelService {
	readonly _serviceBrand: undefined;
	initializeModel(uri: URI): Promise<void>;
	getModel(uri: URI): CodexModelType;
	getModelFromFsPath(fsPath: string): CodexModelType;
	getModelSafe(uri: URI): Promise<CodexModelType>;
	saveModel(uri: URI): Promise<void>;

}

export const ICodexModelService = createDecorator<ICodexModelService>('codexCodexModelService');

class CodexModelService extends Disposable implements ICodexModelService {
	_serviceBrand: undefined;
	static readonly ID = 'codexCodexModelService';
	...
	getModelSafe = async (uri: URI): Promise<CodexModelType> => {
...
registerSingleton(ICodexModelService, CodexModelService, InstantiationType.Eager);
