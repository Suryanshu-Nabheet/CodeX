...
import { FeatureName, ModelSelectionOptions, OverridesOfModel, ProviderName } from './codexSettingsTypes.js';
...
export type CodexStaticModelInfo = { // not stateful
	// Codex uses the information below to know how to handle each model.
...
type CodexStaticProviderInfo = { // doesn't change (not stateful)
	...
	modelOptions: { [key: string]: CodexStaticModelInfo };
	modelOptionsFallback: (modelName: string, fallbackKnownValues?: Partial<CodexStaticModelInfo>) => (CodexStaticModelInfo & { modelName: string, recognizedModelName: string }) | null;
}
...
const defaultModelOptions = {
	...
	supportsFIM: false,
	reasoningCapabilities: false,
} as const satisfies CodexStaticModelInfo
...
} as const satisfies { [s: string]: Partial<CodexStaticModelInfo> }




// keep modelName, but use the fallback's defaults
const extensiveModelOptionsFallback: CodexStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {

	const lower = modelName.toLowerCase()

	const toFallback = <T extends { [s: string]: Omit<CodexStaticModelInfo, 'cost' | 'downloadable'> },>(obj: T, recognizedModelName: string & keyof T)
		: CodexStaticModelInfo & { modelName: string, recognizedModelName: string } => {
...
const anthropicSettings: CodexStaticProviderInfo = {
...
const openAISettings: CodexStaticProviderInfo = {
...
const xAISettings: CodexStaticProviderInfo = {
...
const deepseekSettings: CodexStaticProviderInfo = {
...
const mistralSettings: CodexStaticProviderInfo = {
...
} as const satisfies Record<string, CodexStaticModelInfo>
const googleVertexSettings: CodexStaticProviderInfo = {
...
const microsoftAzureSettings: CodexStaticProviderInfo = {
...
const awsBedrockSettings: CodexStaticProviderInfo = {
...
const vLLMSettings: CodexStaticProviderInfo = {
...
const lmStudioSettings: CodexStaticProviderInfo = {
...
const ollamaSettings: CodexStaticProviderInfo = {
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
...
const openaiCompatible: CodexStaticProviderInfo = {
...
const liteLLMSettings: CodexStaticProviderInfo = { // https://docs.litellm.ai/docs/reasoning_content
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
	modelOptions: {},
	providerReasoningIOSettings: {
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
...
const openRouterSettings: CodexStaticProviderInfo = {
...
const modelSettingsOfProvider: { [providerName in ProviderName]: CodexStaticProviderInfo } = {
