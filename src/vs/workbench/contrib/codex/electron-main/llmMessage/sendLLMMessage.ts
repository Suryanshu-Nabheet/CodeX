...
import { SendLLMMessageParams, OnText, OnFinalMessage, OnError, LLMChatMessage } from '../../common/sendLLMMessageTypes.js';
import { getModelCapabilities } from '../../common/modelCapabilities.js';
import { IMetricsService } from '../../common/metricsService.js';
import { displayInfoOfProviderName } from '../../common/codexSettingsTypes.js';
...
export const sendLLMMessage = async ({
	...
	const onError: OnError = ({ message: errorMessage, fullError }) => {
		...
		if (errorMessage === 'TypeError: fetch failed')
			errorMessage = `Failed to fetch from ${displayInfoOfProviderName(providerName).title}. This likely means you specified the wrong endpoint in Codex's Settings, or your local model provider like Ollama is powered off.`
	...
	try {
		...
		if (messagesType === 'FIMMessage') {
			...
			// Fallback for models that don't support FIM: use chat completion
			const fallbackPrompt = `Complete the code at the location of <CURSOR>.
...
### INSTRUCTIONS:
Provide ONLY the code that should be inserted at the <CURSOR> location.
Do not repeat the prefix or suffix.
Do not include any conversational text or markdown code block markers.
The completion should be seamless.`

			const chatMessages: LLMChatMessage[] = [{ role: 'user', content: fallbackPrompt }]
			await sendChat({ messages: chatMessages, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, overridesOfModel, modelName, _setAborter, providerName, separateSystemMessage: undefined, chatMode: null, mcpTools: undefined })
			return
		}
