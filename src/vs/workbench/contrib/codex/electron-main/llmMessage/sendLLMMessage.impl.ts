...
import { ChatMode, displayInfoOfProviderName, ModelSelectionOptions, OverridesOfModel, ProviderName, SettingsOfProvider } from '../../common/codexSettingsTypes.js';
...
const newOpenAICompatibleSDK = async ({ settingsOfProvider, providerName, includeInPayload }: { settingsOfProvider: SettingsOfProvider, providerName: ProviderName, includeInPayload?: { [s: string]: any } }) => {
	...
	else throw new Error(`Codex providerName was invalid: ${providerName}.`)
}
	...
	openai.chat.completions
		...
		.then(async response => {
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				onError({ message: 'Codex: Response from model was empty.', fullError: null })
			}
			else {
...
const newOllamaSDK = ({ endpoint }: { endpoint: string }) => {
	...
	if (!endpoint) throw new Error(`Ollama Endpoint was empty (please enter ${defaultProviderSettings.ollama.endpoint} in Codex if you want the default url).`)
...
const sendOllamaFIM = ({ messages, onText, onFinalMessage, onError, settingsOfProvider, modelName, _setAborter }: SendFIMParams_Internal) => {
	const thisConfig = settingsOfProvider.ollama
	const ollama = newOllamaSDK({ endpoint: thisConfig.endpoint })

	let fullText = ''

	// Intelligent FIM tag detection for Qwen, Deepseek and other common coding models
	let prompt = messages.prefix
	let suffix = messages.suffix
	let raw = true
	const lowerModel = modelName.toLowerCase()

	if (lowerModel.includes('qwen') || lowerModel.includes('codegemma')) {
		prompt = `<|fim_prefix|>${messages.prefix}<|fim_suffix|>${messages.suffix}<|fim_middle|>`
		suffix = '' // already in prompt
	} else if (lowerModel.includes('deepseek')) {
		prompt = `<｜fim▁begin｜>${messages.prefix}<｜fim▁hole｜>${messages.suffix}<｜fim▁end｜>`
		suffix = ''
	} else if (lowerModel.includes('codestral')) {
		prompt = `[SUFFIX]${messages.suffix}[PREFIX] ${messages.prefix}`
		suffix = ''
	} else {
		raw = false // Use standard Ollama FIM mapping if model is unknown
	}
	...
	ollama.generate({
		...
		options: {
			stop: messages.stopTokens,
			num_predict: 512, // Increased for massive multi-line architecture suggestions
		},
		raw: raw,
		stream: true,
	})
