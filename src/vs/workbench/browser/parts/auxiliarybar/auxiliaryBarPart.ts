...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	...
	override readonly minimumWidth: number = 280; // CodeX changed this (was 170)
	...
	constructor(
		...
		this._register(this.viewDescriptorService.onDidChangeViewContainers(() => this.updateCompositeBar()));
		this._register(this.viewDescriptorService.onDidChangeContainerLocation(() => this.updateCompositeBar()));
		this._register(this.extensionService.onDidRegisterExtensions(() => this.updateCompositeBar()));
	}
	...
	private fillExtraContextMenuActions(actions: IAction[]): void {
		...
		actions.push(...[
			...
			toAction({ id: ToggleSidebarPositionAction.ID, label: currentPositionRight ? localize('move second side bar left', "Move CodeX Side Bar Left") : localize('move second side bar right', "Move CodeX Side Bar Right"), run: () => this.commandService.executeCommand(ToggleSidebarPositionAction.ID) }),
			toggleShowLabelsAction,
			toAction({ id: ToggleAuxiliaryBarAction.ID, label: localize('hide second side bar', "Hide CodeX Side Bar"), run: () => this.commandService.executeCommand(ToggleAuxiliaryBarAction.ID) })
		]);
	}
	...
	protected shouldShowCompositeBar(): boolean {
		...
		for (const container of containers) {
			...
			if (model.allViewDescriptors.length > 0 || !container.hideIfEmpty) {
				visibleContainers++;
			}
		}

		return visibleContainers > 1;
	}

	public override getCompositeBarPosition(): CompositeBarPosition {
