...
export class CompositeOverflowActivityActionViewItem extends CompositeBarActionViewItem {
	...
	showMenu(): void {
		const isSidebar = !!this.container.closest('.part.sidebar');
		if (isSidebar) {
			const action = this.action as CompositeBarAction;
			const item = action.compositeBarActionItem;
			action.compositeBarActionItem = { ...item, classNames: ThemeIcon.asClassNameArray(Codicon.chevronUp) };
		}
		...
		this.contextMenuService.showContextMenu({
			...
			onHide: () => {
				if (isSidebar) {
					const action = this.action as CompositeBarAction;
					const item = action.compositeBarActionItem;
					action.compositeBarActionItem = { ...item, classNames: ThemeIcon.asClassNameArray(Codicon.chevronDown) };
				}
			}
		});
	}
...
export class CompositeActionViewItem extends CompositeBarActionViewItem {
	...
	override render(container: HTMLElement): void {
		...
		this._register(CompositeDragAndDropObserver.INSTANCE.registerDraggable(this.container, () => { return { type: 'composite', id: this.compositeBarActionItem.id }; }, {
			canDrag: () => this.compositeBarActionItem.id !== 'workbench.view.codex',
			onDragOver: e => {
