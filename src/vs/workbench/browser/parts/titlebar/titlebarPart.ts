...
import { Action2, IMenu, IMenuService, MenuId, MenuItemAction, registerAction2, SubmenuItemAction } from '../../../../platform/actions/common/actions.js';
...
export class BrowserTitlebarPart extends Part implements ITitlebarPart {
	...
	private navigationToolBarElement!: HTMLElement;
	...
	protected override createContentArea(parent: HTMLElement): HTMLElement {
		...
		if (hasCustomTitlebar(this.configurationService, this.titleBarStyle)) {
			this.navigationToolBarElement = append(this.leftContent, $('div.titlebar-nav-toolbar'));
	...
	private createActionToolBar() {
		...
		this.navigationToolBar = this.actionToolBarDisposable.add(this.instantiationService.createInstance(WorkbenchToolBar, this.navigationToolBarElement, {
		...
		this.actionToolBar = this.actionToolBarDisposable.add(this.instantiationService.createInstance(WorkbenchToolBar, this.actionToolBarElement, {
			...
			ariaLabel: localize('ariaLabelTitleActions', "Title actions"),
			getKeyBinding: action => this.getKeybinding(action),
			overflowBehavior: { maxItems: 25, exempted: [ACCOUNTS_ACTIVITY_ID, GLOBAL_ACTIVITY_ID] },
	...
	private createActionToolBarMenus(update: true | { editorActions?: boolean; layoutActions?: boolean; activityActions?: boolean } = true) {
		...
		const updateToolBarActions = () => {
			...
			const navigationActions: typeof globalToolbarActions = [];
			const remainingGlobalToolbarActions: typeof globalToolbarActions = [];

			for (const group of globalToolbarActions) {
				if (group[0] === 'navigation') {
					navigationActions.push(group);
				} else {
					remainingGlobalToolbarActions.push(group);
				}
			}

			// --- Navigation Actions
			if (navigationActions.length > 0) {
				const navActions: IToolbarActions = { primary: [], secondary: [] };
				fillInActionBarActions(navigationActions, navActions);
				this.navigationToolBar.setActions(prepareActions(navActions.primary.filter(a => a.id !== 'separator' && !(a instanceof Separator))), prepareActions(navActions.secondary));

			} else {
				this.navigationToolBar.setActions([], []);
			}
			...
			// --- Global Actions
			this.hasGlobalToolbarEntries = remainingGlobalToolbarActions.length > 0;
			fillInActionBarActions(
				remainingGlobalToolbarActions,
				actions
			);
			...
			if (this.layoutToolbarMenu) {
				...
				const filteredLayoutActions = layoutActions.map(group => {
					return [group[0], group[1].filter(action => {
						if (action instanceof MenuItemAction) {
							return action.item.id !== 'workbench.action.toggleSidebarPosition' &&
								action.item.id !== 'workbench.action.customizeLayout';
						}
						if (action instanceof SubmenuItemAction) {
							return action.item.submenu !== MenuId.LayoutControlMenuSubmenu;
						}
						return true;
					})] as [string, Array<MenuItemAction | SubmenuItemAction>];
				});

				fillInActionBarActions(
					filteredLayoutActions,
					actions,
			...
			this.actionToolBar.setActions(prepareActions(actions.primary.filter(a => a.id !== 'separator' && !(a instanceof Separator))), prepareActions(actions.secondary));
