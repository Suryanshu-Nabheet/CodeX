...
export class CompositeDragAndDrop implements ICompositeDragAndDrop {
	...
	drop(data: CompositeDragAndDropData, targetCompositeId: string | undefined, originalEvent: DragEvent, before?: Before2D): void {
		...
		if (dragData.type === 'composite') {
			...
			// CodeX Lock: Prevent moving non-CodeX to AuxiliaryBar, and prevent moving CodeX out of AuxiliaryBar
			if (this.targetContainerLocation === ViewContainerLocation.AuxiliaryBar && dragData.id !== 'workbench.view.codex') {
				return;
			}
			if (currentLocation === ViewContainerLocation.AuxiliaryBar && dragData.id === 'workbench.view.codex' && this.targetContainerLocation !== ViewContainerLocation.AuxiliaryBar) {
				return;
			}
		...
		if (dragData.type === 'view') {
			// CodeX Exclusivity: Prevent moving non-CodeX views to AuxiliaryBar
			if (this.targetContainerLocation === ViewContainerLocation.AuxiliaryBar && dragData.id !== 'workbench.view.codex') {
				return;
			}
...
export class CompositeBar extends Widget implements ICompositeBar {
		...
		this.compositeSwitcherBar = this._register(new ActionBar(actionBarDiv, {
			actionViewItemProvider: (action, options) => {
				...
				return item && this.instantiationService.createInstance(
					...
					{ ...options, draggable: action.id !== 'workbench.view.codex', colors: this.options.colors, icon: this.options.icon, hoverOptions: this.options.activityHoverOptions, compact: this.options.compact },
	...
	private updateCompositeSwitcher(donotTrigger?: boolean): void {
		...
		if (this.options.maxVisibleItems !== undefined) {
			maxVisible = Math.min(maxVisible, this.options.maxVisibleItems);
		}
