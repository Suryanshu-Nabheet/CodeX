...
import { CompositeBarPosition, IPaneCompositePart } from './paneCompositePart.js';
...
export class PaneCompositeBar extends Disposable {
	...
	private getContextMenuActionsForComposite(compositeId: string): IAction[] {
		...
		if (viewContainer.id === 'workbench.view.codex' && this.location === ViewContainerLocation.AuxiliaryBar) {
			return []; // Locked: No move/reset actions for CodeX in Auxiliary Bar
		}
		...
		for (const location of [ViewContainerLocation.Sidebar, ViewContainerLocation.AuxiliaryBar, ViewContainerLocation.Panel]) {
			if (currentLocation !== location) {
				if (location === ViewContainerLocation.AuxiliaryBar && viewContainer.id !== 'workbench.view.codex') {
					continue; // Only CodeX can move to Auxiliary Bar
				}
	...
	private createMoveAction(viewContainer: ViewContainer, newLocation: ViewContainerLocation, defaultLocation: ViewContainerLocation): IAction {
		return toAction({
			...
			label: newLocation === ViewContainerLocation.Panel ? localize('panel', "Panel") : newLocation === ViewContainerLocation.Sidebar ? localize('sidebar', "Primary Side Bar") : localize('auxiliarybar', "CodeX Side Bar"),
			run: () => {
	...
	private addComposite(viewContainer: ViewContainer): void {
		let order = viewContainer.order;
		const pos = this.paneCompositePart.getCompositeBarPosition();
		if (this.part === Parts.SIDEBAR_PART && (pos === CompositeBarPosition.TOP || pos === CompositeBarPosition.BOTTOM)) {
			if (viewContainer.id === 'workbench.view.explorer') {
				order = 0;
			} else if (viewContainer.id === 'workbench.view.search') {
				order = 1;
			} else if (viewContainer.id === 'workbench.view.scm') {
				order = 2;
			} else if (viewContainer.id === 'workbench.view.extensions') {
				order = 3;
			} else if (typeof order === 'number' && order <= 3) {
				order = 4; // Move others (like Debug) out of the first 4 slots
			}
		}
		this.compositeBar.addComposite({ id: viewContainer.id, name: typeof viewContainer.title === 'string' ? viewContainer.title : viewContainer.title.value, order, requestedIndex: viewContainer.requestedIndex });

		// Force Pin the core 4
		if (this.part === Parts.SIDEBAR_PART && (pos === CompositeBarPosition.TOP || pos === CompositeBarPosition.BOTTOM || pos === CompositeBarPosition.TITLE)) {
			if (['workbench.view.explorer', 'workbench.view.search', 'workbench.view.scm', 'workbench.view.extensions'].includes(viewContainer.id)) {
				this.compositeBar.pin(viewContainer.id);
			}
		}
	}
