...
const sidebarLocationIcon = registerIcon('sidebar-location-icon', Codicon.arrowSwap, localize('sidebarLocationIcon', 'Icon represents side bar location configuration.'));

MenuRegistry.appendMenuItem(MenuId.LayoutControlMenu, {
	command: {
		id: ToggleSidebarPositionAction.ID,
		title: localize('toggleSidebarPosition', "Toggle Primary Side Bar Position"),
		icon: sidebarLocationIcon
	},
	group: '1_workbench_layout',
	order: 1, /* Ensure it appears next to the configure icon (default order 0 or undefined) */
...
MenuRegistry.appendMenuItems([{
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar left', "Move CodeX Side Bar Left")
		},
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar right', "Move CodeX Side Bar Right")
		},
			...
			containerModel.visibleViewDescriptors.forEach(viewDescriptor => {
				if (viewDescriptor.canMoveView) {
					if (!hasAddedView) {
						results.push({
							type: 'separator',
							label: localize('secondarySideBarContainer', "CodeX Side Bar / {0}", containerModel.title)
						});
						hasAddedView = true;
					}
	...
	run(accessor: ServicesAccessor, viewId?: string): void {
		...
		if (!(isViewSolo && currentLocation === ViewContainerLocation.AuxiliaryBar)) {
			items.push({
				...
				label: localize('moveFocusedView.newContainerInSidePanel', "New CodeX Side Bar Entry")
			});
		}
		...
		items.push({
			type: 'separator',
			label: localize('secondarySideBar', "CodeX Side Bar")
		});
...
ToggleVisibilityActions.push(...[
	...
	CreateToggleLayoutItem(ToggleAuxiliaryBarAction.ID, AuxiliaryBarVisibleContext, localize('secondarySideBar', "CodeX Side Bar"), { whenA: ContextKeyExpr.equals('config.workbench.sideBar.location', 'left'), iconA: panelRightIcon, iconB: panelLeftIcon }),
