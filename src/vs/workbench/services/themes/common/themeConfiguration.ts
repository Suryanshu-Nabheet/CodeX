...
export class ThemeConfiguration {
	...
	public async setColorTheme(theme: IWorkbenchColorTheme, settingsTarget: ThemeSettingTarget): Promise<IWorkbenchColorTheme> {
		...
		const preferredSettingId = colorSchemeToPreferred[theme.type];
		if (preferredSettingId && preferredSettingId !== this.getColorThemeSettingId()) {
			await this.writeConfiguration(preferredSettingId, theme.settingsId, settingsTarget);
		}
		return theme;
	}
