...
export class ViewDescriptorService extends Disposable implements IViewDescriptorService {
	...
	moveViewContainerToLocation(viewContainer: ViewContainer, location: ViewContainerLocation, requestedIndex?: number, reason?: string): void {
		if (location === ViewContainerLocation.AuxiliaryBar && viewContainer.id !== 'workbench.view.codex') {
			return; // Only allow CodeX in AuxiliaryBar
		}
		if (viewContainer.id === 'workbench.view.codex' && location !== ViewContainerLocation.AuxiliaryBar) {
			return; // Don't allow moving CodeX out of AuxiliaryBar
		}
	...
	moveViewToLocation(view: IViewDescriptor, location: ViewContainerLocation, reason?: string): void {
		if (location === ViewContainerLocation.AuxiliaryBar && view.id !== 'workbench.view.codex') {
			return; // Only allow CodeX in AuxiliaryBar
		}
	...
	moveViewsToContainer(views: IViewDescriptor[], viewContainer: ViewContainer, visibilityState?: ViewVisibilityState, reason?: string): void {
		...
		if (this.getViewContainerLocation(viewContainer) === ViewContainerLocation.AuxiliaryBar && viewContainer.id !== 'workbench.view.codex') {
			return; // Only allow CodeX in AuxiliaryBar
		}
