...
import { transmuteSeverity } from '../transmute';
import { Constants, type DiagnosticTarget } from '../types';
...
export const extUtils = {
	/**
	 * Apply extension settings (`codexErrorLens.messageTemplate`, `codexErrorLens.messageMaxChars`, `codexErrorLens.removeLinebreaks`) to diagnostic message.
	 *
	 ...
	 *
	 * Also, excludes diagnostics according to `codexErrorLens.excludeSources` & `codexErrorLens.exclude` settings.
	 *
	 * Also, apply `codexErrorLens.transmute` setting.
	 *
	...
	shouldExcludeEditor(editor: TextEditor): 'exclude' | 'excludeAndClearDecorations' | 'doNotExclude' {
		...
		if ($config.ignoreDirty && !$state.codexAutosaveEnabled && editor.document.isDirty) {
			return 'excludeAndClearDecorations';
		}
	...
	/**
	 * Whether or not to align editor message text based on values of `codexErrorLens.alignMessage` setting.
	 */
	shouldAlign(): boolean {
	...
	shouldShowInlineMessage(): boolean {
		...
		return extensionEnabled && $state.codexGlobalProblemsEnabled;
	},
