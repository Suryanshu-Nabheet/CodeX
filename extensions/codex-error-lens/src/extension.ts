...
import { disposeAllDecorations, setDecorationStyle, updateDecorationsForAllVisibleEditors } from './decorations';
import { disposeAllEventListeners, updateChangeBreakpointsListener, updateChangeDiagnosticListener, updateChangeVisibleTextEditorsListener, updateChangedActiveTextEditorListener, updateCursorChangeListener, updateOnSaveListener } from './events';
import { StatusBarIcons } from './statusBar/statusBarIcons';
import { StatusBarMessage } from './statusBar/statusBarMessage';
import { Constants, type ExtensionConfig } from './types';
...
export abstract class $state {
	...
	/**
	 * CodeX `problems.visibility` setting value.
	 */
	static codexGlobalProblemsEnabled = true;
	/**
	 * CodeX `files.autoSave` setting value.
	 */
	static codexAutosaveEnabled = false;
	/**
	...
	/**
	 * `true` when use specified anything inide of `codexErrorLens.transmute` setting.
	 */
...
export function activate(context: ExtensionContext): void {
	...
	function updateConfigAndEverything(): void {
		...
		$state.codexGlobalProblemsEnabled = workspace.getConfiguration('problems').get<boolean>('visibility') ?? true;
		$state.codexAutosaveEnabled = workspace.getConfiguration('files').get<string>('autoSave') === 'afterDelay';
		disposeEverything();
		if ($config.enabled) {
