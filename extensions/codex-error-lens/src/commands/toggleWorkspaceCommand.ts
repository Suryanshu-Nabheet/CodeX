...
export async function toggleWorkspaceCommand(): Promise<void> {
	...
	if ($config.excludeWorkspaces?.includes(workspaceFsPath)) {
		...
		showResultNotification(`"${workspaceFsPath}" - Removed from "codexErrorLens.excludeWorkspaces"`);
	} else {
		newExcludeWorkspaceList = [...($config.excludeWorkspaces || []), workspaceFsPath];
		showResultNotification(`"${workspaceFsPath}" - Added to "codexErrorLens.excludeWorkspaces" (now ignored)`);
	}

	vscodeUtils.updateGlobalSetting('codexErrorLens.excludeWorkspaces', newExcludeWorkspaceList);
}
