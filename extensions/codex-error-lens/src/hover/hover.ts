...
import { type RuleDefinitionArgs } from '../commands/findLinterRuleDefinitionCommand';
...
export function createHoverForDiagnostic({
	...
	if (buttonsEnabled) {
		const excludeProblemButton = vscodeUtils.createButtonLinkMarkdown({
			...
			title: 'Exclude problem from CodeX Error Lens by source/code',
		});
		...
		const searchForProblemButton = vscodeUtils.createButtonLinkMarkdown({
			...
			title: 'Open problem in default browser (controlled by `codexErrorLens.searchForProblemQuery` setting).',
		});
