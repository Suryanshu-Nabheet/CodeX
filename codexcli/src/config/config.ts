...
import { providers, normalizeProviderKey } from "./providers.js";
...
export const DEFAULT_AGENTIC_MODEL = "";
export const DEFAULT_FULL_CONTEXT_MODEL = "";
export const DEFAULT_APPROVAL_MODE = AutoApprovalMode.AUTO_EDIT;
...
export const LLM_TIMEOUT_MS =
  parseInt(process.env["LLM_TIMEOUT_MS"] || "0", 10) || undefined;
export const LLM_BASE_URL = process.env["LLM_BASE_URL"] || "";
export let LLM_API_KEY = process.env["LLM_API_KEY"] || "";

export const DEFAULT_REASONING_EFFORT = "high";
export const LLM_ORGANIZATION = process.env["LLM_ORGANIZATION"] || "";
export const LLM_PROJECT = process.env["LLM_PROJECT"] || "";

// Can be set `true` when CodexCLI is running in an environment that is marked as already
...
export function getBaseUrl(
  provider: string = "codexcli",
): string | undefined {
  const normalized = normalizeProviderKey(provider);
  // Check for a PROVIDER-specific override: e.g. ANTHROPIC_BASE_URL or OLLAMA_BASE_URL.
  const envKey = `${normalized.toUpperCase()}_BASE_URL`;
  ...
  const providerInfo = providersConfig[normalized];
  if (providerInfo) {
    return providerInfo.baseURL;
  }

  // If the provider not found in the providers list and `LLM_BASE_URL` is set, use it.
  if (LLM_BASE_URL !== "") {
    return LLM_BASE_URL;
  }
...
export function getApiKey(
  ...
  const normalized = normalizeProviderKey(provider);
  const config = loadConfig();
  const providersConfig = { ...providers, ...(config.providers ?? {}) };
  const providerInfo = providersConfig[normalized];

  if (normalized === "ollama") {
    const envName = providerInfo?.envKey ?? "OLLAMA_API_KEY";
    return process.env[envName]?.trim() || config.apiKey?.trim() || "ollama";
  }

  if (providerInfo) {
    const fromEnv = process.env[providerInfo.envKey]?.trim();
    if (fromEnv) {
      return fromEnv;
    }
  }
  ...
  const customApiKey = process.env[`${normalized.toUpperCase()}_API_KEY`]?.trim();
  if (customApiKey) {
    return customApiKey;
  }
  ...
  // Shared LLM_API_KEY fallback
  if (LLM_API_KEY !== "") {
    return LLM_API_KEY;
  }

  // Config / setup wizard persistence
...
export function loadProjectDoc(cwd: string, explicitPath?: string): string {
  ...
  if (explicitPath) {
    ...
    if (!existsSync(filepath)) {
      ...
      console.warn(`CodexCLI: project doc not found at ${filepath}`);
      filepath = null;
    }
  } else {
  ...
  try {
    ...
    if (buf.byteLength > PROJECT_DOC_MAX_BYTES) {
      // eslint-disable-next-line no-console
      console.warn(
        `CodexCLI: project doc '${filepath}' exceeds ${PROJECT_DOC_MAX_BYTES} bytes – truncating.`,
      );
    }
...
export const loadConfig = (
  ...
  if (
    ...
    if (storedConfig.disableResponseStorage === "true") {
      ...
      log(
        `[CodexCLI] Warning: 'disableResponseStorage' in config is not a boolean (got '${storedConfig.disableResponseStorage}'). Ignoring this value.`,
      );
  ...
  if (shouldLoadProjectDoc) {
    ...
    if (projectDocPath) {
      log(
        `[CodexCLI] Loaded project doc from ${projectDocPath} (${projectDoc.length} bytes)`,
      );
    } else {
      log(`[CodexCLI] No project doc found in ${cwd}`);
    }
  }
  ...
  const providerKey = normalizeProviderKey(storedConfig.provider ?? "codexcli");
  const mergedProviders = { ...providers, ...(storedConfig.providers ?? {}) };
  const providerEnvKey = mergedProviders[providerKey]?.envKey ?? "LLM_API_KEY";

  const config: AppConfig = {
    model:
      storedModel ??
      (options.isFullContext
        ? DEFAULT_FULL_CONTEXT_MODEL || "default-model"
        : DEFAULT_AGENTIC_MODEL || "default-model"),
    ...
    apiKey:
      process.env[providerEnvKey]?.trim() ||
  ...
  try {
    if (!existsSync(actualConfigPath)) {
      ...
      // pinning users to an old default after upgrading CodexCLI.
      const ext = extname(actualConfigPath).toLowerCase();
      if (ext === ".yaml" || ext === ".yml") {
