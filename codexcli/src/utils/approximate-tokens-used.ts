    ...
    switch (item.type) {
        ...
        if (item.content) {
          for (const c of item.content) {
            if (c.type === "input_text" || c.type === "output_text") {
              charCount += (c as any).text.length;
            } else if (c.type === "refusal") {
              charCount += (c as any).refusal.length;
            } else if (c.type === "input_file") {
              charCount += (c as any).filename?.length ?? 0;
            }
      ...
      case "function_call": {
        charCount +=
          ...
          ((item as any).arguments?.length || 0);
        break;
      }

      case "function_call_output": {
        charCount += (item as any).output?.length || 0;
        break;
      }
