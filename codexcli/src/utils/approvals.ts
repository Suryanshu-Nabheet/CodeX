...
export function canAutoApprove(
  ...
  if (
    ...
    // Under auto-edit, allow simple verify scripts inside `bash -lc '…'`
    if (policy === "auto-edit") {
      const simple = toSimpleCommand(bashCmd);
      const verify = simple ? isVerifyCommand(simple) : null;
      if (verify != null) {
        return {
          type: "auto-approve",
  ...
  const verifyOuter = policy === "auto-edit" ? isVerifyCommand(command) : null;
  return policy === "full-auto"
    ...
    : verifyOuter != null
      ? {
          type: "auto-approve",
          reason: verifyOuter.reason,
          group: verifyOuter.group,
          runInSandbox: false,
        }
      : { type: "ask-user" };
}
...
/**
 * Build/test/typecheck commands that are safe to auto-run under auto-edit
 * so the agent can verify its own work without stopping every turn.
 */
export function isVerifyCommand(
  command: ReadonlyArray<string>,
): SafeCommandReason | null {
  const [cmd0, cmd1, cmd2] = command;
  ...
  if (
    ...
    if (cmd1 === "test" || cmd1 === "lint" || cmd1 === "typecheck") {
      return { reason: `${cmd0} ${cmd1}`, group: "Verification" };
    }
    if (
      (cmd1 === "run" || cmd1 === "exec") &&
      typeof cmd2 === "string" &&
      VERIFY_SCRIPT_NAMES.has(cmd2)
    ) {
      return {
        reason: `${cmd0} ${cmd1} ${cmd2}`,
        group: "Verification",
      };
    }
  }

  // Direct compilers / linters
  if (
    cmd0 === "tsc" ||
    cmd0 === "eslint" ||
    cmd0 === "prettier" ||
    ...
    if (cmd0 === "cargo" && cmd1 !== "check" && cmd1 !== "test" && cmd1 !== "clippy") {
      return null;
    }
    if (cmd0 === "prettier" && cmd1 === "--write") {
      return null; // mutating format — still ask under auto-edit
    }
    return { reason: command.join(" "), group: "Verification" };
  }

  // npx tsc / npx eslint / npx vitest (read-only / test)
  if (cmd0 === "npx" && typeof cmd1 === "string") {
    if (
      cmd1 === "tsc" ||
      cmd1 === "eslint" ||
      cmd1 === "vitest" ||
      ...
      if (cmd1 === "prettier" && command.includes("--write")) {
        return null;
      }
      return { reason: command.join(" "), group: "Verification" };
    }
...
const VERIFY_SCRIPT_NAMES: ReadonlySet<string> = new Set([
  "test",
  "test:unit",
  "test:watch",
  "lint",
...
/** Flatten a shell-quote parse result into a simple argv if it has no operators. */
function toSimpleCommand(
  parsed: ReadonlyArray<ParseEntry>,
): Array<string> | null {
  ...
  for (const part of parsed) {
    ...
    // Operators / redirects / globs — not a simple command
    return null;
  }
