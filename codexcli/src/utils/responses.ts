...
// Function to convert input items to messages
...
function convertInputItemToMessage(
  ...
  const responseItem = item as any;

  if (responseItem.type === "message") {
    const role = responseItem.role;
    ...
    responseItem.type === "local_shell_call"
  ) {
    const call_id =
      responseItem.call_id ||
      responseItem.id ||
      "call_" + Math.random().toString(36).substring(2, 9);
    return {
      role: "assistant",
      ...
      tool_calls: [
        {
          ...
          function: {
            name:
              responseItem.name ||
              (responseItem.action?.type === "exec"
                ? "run_command"
                : "unknown"),
            arguments:
              typeof responseItem.arguments === "string"
                ? responseItem.arguments
                : JSON.stringify(
                    responseItem.arguments || responseItem.action || {},
                  ),
          },
        },
      ],
    ...
    return {
      role: "tool",
      tool_call_id: responseItem.call_id || responseItem.id,
      content: responseItem.output || "",
    } as OpenAI.Chat.Completions.ChatCompletionToolMessageParam;
  } else if (responseItem.type === "reasoning") {
    return {
      role: "assistant",
      content: responseItem.summary || "",
    } as OpenAI.Chat.Completions.ChatCompletionAssistantMessageParam;
  }

  // Fallback for unknown types
  return { role: "user", content: JSON.stringify(responseItem) } as any;
}
...
function getFullMessages(
  ...
  const newInputMessages = Array.isArray(input.input)
    ? (input.input as Array<any>).map(convertInputItemToMessage)
    : [convertInputItemToMessage(input.input as any)];
...
const createCompletion = (llmClient: OpenAI, input: ResponseCreateInput) => {
  ...
  const chatInput: OpenAI.Chat.Completions.ChatCompletionCreateParams = {
    ...
    ...(input.reasoning?.effort
      ? { reasoning_effort: input.reasoning.effort as any }
      : {}),
  };

  return llmClient.chat.completions.create(chatInput);
};
...
async function responsesCreateViaChatCompletions(
  ...
  const completion = await createCompletion(llmClient, input);
  if (input.stream) {
    ...
    if (choice.finish_reason) {
      if (isToolCall) {
        for (const [tcIndex, tc] of toolCalls) {
          if (
            !finalOutputItem.some(
              (i) => i.type === "function_call" && (i as any).call_id === tc.id,
            )
          ) {
            const item = {
              type: "function_call",
...
export type ResponseItem = ResponseItemType;
export type ResponseFunctionToolCallItem = ResponseItemType;
export type ResponseFunctionToolCallOutputItem = ResponseItemType;
export type ResponseInputMessageItem = ResponseItemType;
export type ResponseReasoningItem = ResponseItemType;
export type ResponseOutputMessage = ResponseItemType;
...
export type ResponseInputItem =
  ...
  | { type: "reasoning"; summary?: string; effort?: string; [key: string]: any }
  | { type: "local_shell_call"; [key: string]: any }
  | { type: string; [key: string]: any };

export type ResponseFunctionToolCall = ResponseInputItem & {
  type: "function_call";
  id: string;
  call_id: string;
  name: string;
...
export {
  ...
  ResponseCreateInput as ResponseCreateParams,
  ResponseOutput,
  ResponseEvent,
};
