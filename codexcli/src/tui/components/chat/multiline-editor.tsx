...
export interface MultilineTextEditorProps {
  ...
  /** Full-width bar fill (e.g. blackBright) behind each editor line. */
  readonly backgroundColor?: string;

  /** Prompt shown on the first line (e.g. "> "); later lines are indented. */
    ...
    <Box flexDirection="column" key={version}>
      {visibleLines.map((lineText, idx) => {
        ...
        const gutterNode =
          gutter.length > 0 ? (
            idx === 0 ? (
              <Text bold color={gutterColor}>
                {gutter}
              </Text>
            ) : (
              <Text>{" ".repeat(gutter.length)}</Text>
            )
          ) : null;
